...
function textureCards(list){return `<div class="catalog">${list.map(item=>`<a class="card texture" href="#texture/${encodeURIComponent(item.id)}">${ItemIcons.render(item.id,item.name)}<strong>${esc(item.name)}</strong><small>${esc(item.group)} &middot; ${item.file?'Verified inventory icon':'Icon unavailable'}</small></a>`).join('')}</div>`;}
function legacyTextureKey(key){return /^texture\/[a-z0-9_.%~-]+\.png$/i.test(key);}
function catalogItem(id){
  return catalog.find(x=>x.id===id) || catalog.find(x=>x.id.replace(':','_')+'.png'===id);
}
function iconDetail(id,path){
  const item=catalogItem(id);
  if(!item){
    main.innerHTML=heading('Icon unavailable','This former texture record has no verified matching inventory item. Saved notes remain available.')+ItemIcons.placeholder(id||'Unknown item')+bookmark(path)+notes(path)+link('catalog','Browse inventory icons','button');
    return;
  }
  const icon=ItemIcons.get(item.id);
  const source=(window.INVENTORY_ICON_SOURCES||[]).find(s=>s.id===icon.source);
  main.innerHTML=heading(item.name,icon.status==='verified'?'Verified inventory icon':'Icon unavailable','Inventory catalog')+`<div class="row">${ItemIcons.render(item.id,item.name,'detail-image')}${bookmark(path)}</div><h2>Verification</h2><p>Item ID: <code>${esc(item.id)}</code><br>Minecraft: ${esc(icon.minecraft)} &middot; Mod version: ${esc(icon.version||'Unresolved')}</p><p>${icon.status==='verified'?'Static inventory sprite checked against its exact item model.':esc(icon.reason)}</p>${icon.sourceUrl?external(icon.sourceUrl,'Versioned source'):''}${source?.license?`<p>Artwork: ${esc(source.name)} &middot; ${source.license.url?external(source.license.url,source.license.id):esc(source.license.id)}. Rights remain with the original authors.</p>`:''}<p>Item models and translations identify artwork; obtainability and recipes have not been verified against your installed pack.</p>${link('catalog?'+new URLSearchParams({group:item.group}),'Browse this mod','button')}${notes(path)}`;
}
function catalogPage(params){
  ...
  main.innerHTML=heading('Inventory icons','Vanilla and mod inventory artwork checked against Minecraft 1.20.1 item models. Items needing an unverified render show an unavailable placeholder.')+`<form id="catalog-form" class="row"><label for="catalog-search">Search inventory icons</label><input id="catalog-search" name="q" type="search" value="${esc(q)}" placeholder="e.g. sword @minecraft -netherite"><label for="group">Mod namespace</label><select id="group" name="group"><option value="">All mods</option>${[...new Set(catalog.map(x=>x.group))].sort().map(g=>`<option ${g===group?'selected':''} value="${esc(g)}">${esc(g)}</option>`).join('')}</select><button>Apply filters</button>${link('catalog','Clear filters')}</form><p class="muted" role="status">${filtered.length.toLocaleString()} items · Page ${page} of ${pages}</p>${filtered.length?textureCards(filtered.slice((page-1)*60,page*60)):'<div class="empty">No matching items. Try fewer words or clear the mod filter.</div>'}<div class="row pagination">${page>1?link(pageLink(1),'First','button')+link(pageLink(page-1),'Previous','button'):'<button disabled>Previous</button>'}${page<pages?link(pageLink(page+1),'Next','button')+link(pageLink(pages),'Last','button'):'<button disabled>Next</button>'}</div>`;
}
...
main.addEventListener('change',async e=>{
  ...
  if(e.target.id==='import'){
    try{
      ...
      const valid=key=>typeof key==='string'&&(articles.some(a=>key==='article/'+a.id)||mods.some(m=>key==='mod/'+m.id)||catalog.some(x=>key==='texture/'+encodeURIComponent(x.id))||legacyTextureKey(key));
